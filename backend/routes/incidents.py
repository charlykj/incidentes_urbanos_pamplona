...
def crear_incidente(
    ...
    if duplicados:
        return {
            ...
            "mensaje": "Se encontraron incidentes similares cercanos.",
        }
    ...
    entidad_final = data.entidad_asignada or ENTIDADES_MAP.get(data.categoria, "")
    ...
    log_audit(usuario_id=usuario_id, usuario_nombre=usuario_nombre, accion="creacion", entidad_id=uid, detalle={"categoria": data.categoria})
    return {"duplicados_encontrados": False, "mensaje": "Incidente creado", "id": uid, "CiudadZona": pk, "FechaID": sk}


@router.post("/{incidente_id}/confirm")
def confirmar_incidente(incidente_id: str, data: ConfirmacionRequest, current_user=Depends(get_current_user)):
    ...
    log_audit(usuario_id=data.usuario_id, usuario_nombre=data.usuario_nombre, accion="confirmacion", entidad_id=incidente_id)
    return {"mensaje": "Confirmación registrada", "confirmaciones": len(confirmados)}


@router.post("/{incidente_id}/evidence")
def agregar_evidencia(incidente_id: str, data: EvidenciaRequest, current_user=Depends(get_current_user)):
    item = _get_incidente_by_id(incidente_id)
    if not item:
        raise HTTPException(404, "Incidente no encontrado")
    multimedia = item.get("multimedia", [])
    multimedia.append({"url": data.url, "tipo": data.tipo, "fecha": datetime.utcnow().isoformat(), "usuario_id": data.usuario_id})
    tabla = get_table()
    tabla.update_item(
        Key={"CiudadZona": item["CiudadZona"], "FechaID": item["FechaID"]},
...
@router.get("/")
def listar_incidentes(estado: Optional[str] = Query(None), categoria: Optional[str] = Query(None)):
    tabla = get_table()
    if estado:
        result = tabla.query(IndexName="GSI-estado", KeyConditionExpression=Key("estado").eq(estado))
    elif categoria:
        result = tabla.query(IndexName="GSI-categoria", KeyConditionExpression=Key("categoria").eq(categoria))
    else:
        result = tabla.scan()
...
def historial_incidente(incidente_id: str):
    ...
    if not item:
        raise HTTPException(404, "Incidente no encontrado")
    return {"incidente_id": incidente_id, "estado_actual": item.get("estado"), "historial": item.get("historial_estados", [])}


@router.put("/{incidente_id}/status")
def cambiar_estado(incidente_id: str, data: CambioEstadoRequest, current_user=Depends(require_role("operador", "supervisor", "admin"))):
    ...
    historial.append({"estado": nuevo_estado, "fecha": now, "usuario_id": current_user["usuario_id"], "usuario_nombre": current_user["nombre"], "observacion": data.observacion or ""})
    tabla = get_table()
    tabla.update_item(
    ...
    log_audit(usuario_id=current_user["usuario_id"], usuario_nombre=current_user["nombre"], accion="cambio_estado", entidad_id=incidente_id, detalle={"de": estado_actual, "a": nuevo_estado})
    return {"mensaje": "Estado actualizado"}


@router.delete("/{incidente_id}")
def eliminar_incidente(incidente_id: str, ciudad_zona: Optional[str] = Query(None), fecha_id: Optional[str] = Query(None), current_user=Depends(require_role("supervisor", "admin"))):
    ...
    log_audit(usuario_id=current_user["usuario_id"], usuario_nombre=current_user["nombre"], accion="eliminacion", entidad_id=incidente_id)
